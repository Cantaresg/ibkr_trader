"""
Head-to-head evaluation of the 2x2 full-dataset comparison checkpoints:
  ppo_full_nosyn   — PPO, no synthetic augmentation
  ppo_full_syn50   — PPO, 50% synthetic bear augmentation
  rppo_full_nosyn  — RecurrentPPO, no synthetic augmentation
  rppo_full_syn50  — RecurrentPPO, 50% synthetic bear augmentation (best overall)

Evaluates each on three out-of-sample periods: 2019 Val, 2020 COVID, 2022 Fed.
All checkpoints were trained on 2009-2021 with episode_length=252.

Usage:
    python scripts/compare_ppo_rppo.py
    python scripts/compare_ppo_rppo.py --n-episodes 40
    python scripts/compare_ppo_rppo.py --out results/ppo_rppo_comparison.csv
"""
import sys
import argparse
import csv
...
CHECKPOINTS = [
    {
        "label":  "ppo_nosyn",
        "path":   "checkpoints/ppo_full_nosyn/best/best_model.zip",
        "ep_len": 252,
    },
    {
        "label":  "ppo_syn50",
        "path":   "checkpoints/ppo_full_syn50/best/best_model.zip",
        "ep_len": 252,
    },
    {
        "label":  "rppo_nosyn",
        "path":   "checkpoints/rppo_full_nosyn/best/best_model.zip",
        "ep_len": 252,
    },
    {
        "label":  "rppo_syn50 (BEST)",
        "path":   "checkpoints/rppo_full_syn50/best/best_model.zip",
        "ep_len": 252,
    },
]
...
def _load_model(path: str):
    # Try RecurrentPPO first so RPPO checkpoints aren't silently loaded as PPO
    if _RPPO_AVAILABLE:
        try:
    ...
    try:
        ...
        pass
    raise ValueError(f"Cannot load checkpoint: {path}")
...
def evaluate_period(model, is_recurrent, data_store, cfg, eval_start, eval_end, ep_len, n_episodes, seed):
...
def parse_args():
    ...
    p.add_argument("--out",        default="results/ppo_rppo_comparison.csv")
    return p.parse_args()
...
def main():
    ...
    for ckpt in CHECKPOINTS:
        path = Path(ckpt["path"])
        if not path.exists():
            print(f"SKIP {ckpt['label']} — not found: {path}")
            continue

        print(f"\nLoading {ckpt['label']}...")
        model, is_recurrent = _load_model(str(path))
        algo_type = "RPPO" if is_recurrent else "PPO"
        print(f"  Detected: {algo_type}")

        for eval_start, eval_end, period_label in PERIODS:
            print(f"  Evaluating {period_label}...", end=" ", flush=True)
            r = evaluate_period(
                model, is_recurrent, data_store, cfg,
                eval_start, eval_end,
                ckpt["ep_len"],
                args.n_episodes,
                args.seed,
            ...
            rows.append({
                ...
                "reward":     round(r["mean_reward"],        4),
            })

    # --- Print comparison table ---
    W = 110
    print("\n" + "=" * W)
    print(f"  {'Model':<22}  {'Period':<14}  {'Return':>8}  {'StdDev':>7}  {'Win%':>5}  {'MaxDD%':>7}  {'Sharpe':>7}  {'Reward':>8}")
    print("=" * W)
    last = None
    for row in rows:
        if row["model"] != last and last is not None:
            print("-" * W)
        last = row["model"]
        print(
            f"  {row['model']:<22}  {row['period']:<14}  "
            f"{row['return_pct']:>+7.2f}%  {row['std_pct']:>6.2f}%  "
            f"{row['win_rate']:>5.0f}%  {row['mean_maxdd']:>6.1f}%  "
            f"{row['sharpe']:>+7.3f}  {row['reward']:>+8.4f}"
        )
    print("=" * W)

    # --- Bear-market summary ---
    bear_periods = {"2020 COVID", "2022 Fed"}
    print("\nBear-market summary (avg of 2020 COVID + 2022 Fed):")
    for ckpt in CHECKPOINTS:
        label     = ckpt["label"]
        ...
        if bear_rows:
            ...
            print(f"  {label:<22}  val={val_ret:>+6.2f}%  bear_avg={bear_ret:>+6.2f}%  bear_sharpe={bear_sharpe:>+.3f}")
